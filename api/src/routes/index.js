...
router.get("/pokemons", async (req, res) => {
  ...
  try {
    ...
    if (name) {
      ...
      ); // usamos includes porque getAllPokemons retorna un array con objetos, yo quiero hacerle un filter a la variable donde tengo dicho array (pokemonsTotal) y quedarme solo con el objeto cuyo nombre coincida con el filter.
      pokemonName.length
...
router.put("/pokemons/:id", async (req, res) => {
  try {
    ...
    res.status(200).send((message = { message: "pokemon actualizado" }));
  } catch (error) {
    console.log(error);
  }
});
